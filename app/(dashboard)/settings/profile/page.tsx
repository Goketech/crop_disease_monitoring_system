...
export default function ProfileSettingsPage() {
  const {
    ...
    smsNotificationsEnabled: initialSms,
  } = useRole();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Uploading to 'plant-images' bucket (coz it already exists from previous steps)
    ...
    <div className="flex flex-col gap-8 w-full max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-black tracking-tighter text-[#1A5336]">
            Profile Settings
          </h1>
          <p className="text-sm text-muted-foreground font-medium italic">
            Manage your identity on the PlantMD platform.
          </p>
        </div>
      </div>
      ...
      <Card className="p-8 rounded-[32px] bg-white border-none shadow-xl shadow-black/5 flex flex-col gap-10 overflow-hidden relative">
        ...
        <div className="flex items-center gap-8 pb-8 border-b border-border/40">
          ...
          <div className="flex flex-col gap-3">
            <h3 className="font-black text-xl text-[#1A5336] leading-tight">
              Your Avatar
            </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12">
          ...
          <div className="flex flex-col gap-3">
            <label className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1 flex items-center gap-2">
              <Mail className="h-3.5 w-3.5 text-primary opacity-50" /> Email
              (Read-only)
            </label>
            <Input
              disabled
